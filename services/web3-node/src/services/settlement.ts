...
export class SimulatedSettlement implements SettlementProvider {
  ...
  get network(): string {
    return this.config.network === 'web3-ledger' ? 'sim-stablecoin' : this.config.network;
  }
  async settle(intent: SettlementIntent): Promise<SettlementResult> {
  ...
  async settle(intent: SettlementIntent): Promise<SettlementResult> {
    ...
    if (!token) {
      return {
        ...
        status: 'failed',
        detail: 'testnet settlement needs WEB3_SETTLEMENT_TOKEN (an ERC-20 address)',
      };
    }
