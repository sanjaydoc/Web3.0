 ...
 * dashboard rather than .env. If ACP_ADMIN_TOKEN is set (recommended in production), a matching
...
export function adminRequired(): boolean {
  return Boolean(process.env.ACP_ADMIN_TOKEN);
}
...
export function checkAdmin(request: FastifyRequest, reply: FastifyReply): boolean {
  const expected = process.env.ACP_ADMIN_TOKEN;
