...
/**
 * Host-LLM tunnel — the operator publishes local models they host (Ollama tags) and the price they
 * charge per million tokens. Each offer is tunnel-served so an agent owner can pick it as a hosted
 * brain from the Marketplace. Rows show the models this operator hosts; add/remove manage the catalogue.
 */
function LlmTunnelCard() {
  ...
  return (
    <div className="card" style={{ marginBottom: 18 }}>
      <div className="section-title">Host LLM tunnel · sell inference</div>
      <p className="muted" style={{ margin: '2px 0 12px' }}>
        Host local models on this machine and sell inference over the relay. Each model you publish
        appears in the agent-owner Marketplace as a hosted brain. Runs your node's local model
        client (Ollama) — pull the tag first, then publish it here.
      </p>
      <dl className="kv">
