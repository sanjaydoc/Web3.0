...
import { type HostingSummary, NODE_URL, api } from './api.js';
...
export function Hosting() {
  ...
  const load = useCallback(async () => {
    try {
      setSummary(await api.hostingSummary());
    } catch {
      ...
      >
        <div className="card" style={{ marginBottom: 18 }}>
          ...
          <dl className="kv">
            ...
            <dt>Rent earned</dt>
            <dd>
              <span className="muted">metered per epoch — arrives with billing (R2)</span>
            </dd>
          </dl>
        </div>
